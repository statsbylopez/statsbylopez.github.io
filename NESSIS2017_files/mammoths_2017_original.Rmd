...
## Example: Major League Baseball, 2016-17

```{r}
library(gsheet)
library("BradleyTerry2")
urlA <- "https://docs.google.com/spreadsheets/d/1W9TjBmt34vR-tCMV5SFAWQUmUeYBtBXNylLreJ-hJQ4/edit?usp=sharing"
p_A <- gsheet2tbl(urlA)
teams <- p_A %>% group_by(Away) %>% count() %>% filter(n > 4)
nescac.games <- p_A %>% filter(Away %in% teams$Away & Home %in% teams$Away)
set.seed(0)
```

```{r, echo = TRUE}
sample_n(nescac.games, 3)
```


## Example: NESCAC men's basketball, 2016-17

**Team strength estimates**

```{r, fig.width = 8, fig.height = 4.5}
result <- nescac.games$HomeScore > nescac.games$AwayScore
homeBT <- BTm(result,
              data.frame(team = nescac.games$Home, home.adv = 1),
              data.frame(team = nescac.games$Away, home.adv = 0),
              ~ team + home.adv,
              id = "team")

df.predict <- data.frame(exp(BTabilities(homeBT)))
...
mammoth_probs <- df.predict %>%
      filter (Team != "Amherst") %>%
      mutate (p.Amherst = exp(1)/(exp(1) + exp(ability)))

p <- ggplot(mammoth_probs, aes(x = reorder(Team, p.Amherst))) + geom_bar(aes(weight = p.Amherst)) +
  ylab("") + xlab("") + theme_bw() +
  geom_hline(aes(yintercept = 0.5), color = "red", lty = 2) +
  labs(title = "Probability of an Amherst win", subtitle = "2016-2017 men's basketball")  + 
  scale_y_continuous(labels = scales::percent) + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme_gray(14)
p
...
**Home advantage: $\hat{\alpha} = 0.69$**

*Probability of beating a equally talented team at home is about $\frac{e^{\hat{\alpha}}}{1 + e^{\hat{\alpha}}} = \frac{e^{0.69}}{1 + e^{0.69}}$ = 66.5%*
